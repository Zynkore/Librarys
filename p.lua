...
-- Bundled by Claude AI (Fluent-modified)
local __modules = {}
local __loaded  = {}

local function __require(key)
    if __loaded[key] ~= nil then return __loaded[key] end
    local mod = __modules[key]
    if not mod then error("Module not found: " .. tostring(key)) end
    local result = mod()
    __loaded[key] = result == nil and true or result
    return __loaded[key]
end
...
return function(props)
	...
	if __require("__root__").UseAcrylic then
		Blur = AcrylicBlur()
...
function Dialog:Create()
	...
	function NewDialog:Open()
		__require("__root__").DialogOpen = true
	...
	function NewDialog:Close()
		__require("__root__").DialogOpen = false
		TintTransparency(1)
		RootTransparency(1)
		Scale(1.1)
	...
	function NewDialog:Button(Title, Callback)
		...
		Creator.AddSignal(Button.Frame.MouseButton1Click, function()
			__require("__root__"):SafeCallback(Callback)
			pcall(function()
				NewDialog:Close()
			end)
		end)
...
return function(Title, Desc, Parent, Hover)
	...
	if Hover then
		-- (bundled: Themes resolved via __require)
...
local Spring = Flipper.Spring.new
local Instant = Flipper.Instant.new
-- (bundled: Components resolved via __require)
...
function TabModule:New(Title, Icon, Parent)
	local Library = __require("__root__")
	...
	function Tab:AddSection(SectionTitle)
		...
		local SectionFrame = __require("Components.Section")(SectionTitle, Tab.Container)
...
return function(Config)
	...
	local DialogModule = __require("Components.Dialog"):Init(Window)
	...
	local TabModule = __require("Components.Tab"):Init(Window)
...
function Creator.GetThemeProperty(Property)
	if Themes[__require("__root__").Theme][Property] then
		return Themes[__require("__root__").Theme][Property]
	end
...
function Creator.SpringMotor(Initial, Instance, Prop, IgnoreDialogCheck, ResetOnThemeChange)
	...
	local function SetValue(Value, Ignore)
		...
		if not IgnoreDialogCheck then
			if not Ignore then
				if Prop == "BackgroundTransparency" and __require("__root__").DialogOpen then
					return
				end
			end
		end
...
for _, Theme in next, {__require("Elements.Button"), __require("Elements.Colorpicker"), __require("Elements.Divider"), __require("Elements.Dropdown"), __require("Elements.Input"), __require("Elements.Keybind"), __require("Elements.Paragraph"), __require("Elements.Slider"), __require("Elements.Toggle")} do
	table.insert(Elements, Theme)
end
...
function Element:New(Config)
	...
	local ButtonFrame = __require("Components.Element")(Config.Title, Config.Description, self.Container, true)
	...
	-- Ripple overlay for touch feedback
	...
	local function PlayRipple()
		RippleOverlay.BackgroundTransparency = 0.82
		TweenService:Create(
			RippleOverlay,
	...
	Creator.AddSignal(ButtonFrame.Frame.InputEnded, function(Input)
		if
			(Input.UserInputType == Enum.UserInputType.Touch
			or Input.UserInputType == Enum.UserInputType.MouseButton1)
			and isTouching
...
function Element:New(Idx, Config)
	...
	local ColorpickerFrame = __require("Components.Element")(Config.Title, Config.Description, self.Container, true)
	...
	local function CreateColorDialog()
		local Dialog = __require("Components.Dialog"):Create()
		...
		local function CreateInput()
			local Box = __require("Components.Textbox")()
		...
		local function Display()
			...
			if Config.Transparency then
				...
				AlphaInput.Input.Text = __require("__root__"):Round((1 - Transparency) * 100, 0) .. "%"
			end
		end
...
--[[
    Divider — Fluent UI Element
    Adds a horizontal visual separator between elements in a tab or section.

    Usage:
        Tab:AddDivider()              -- plain full-width line
        Tab:AddDivider("Combate")     -- line with centred label; gaps on each side
]]
...
local SIDE_GAP = 8  -- pixels between the line ends and the label

local Element = {}
Element.__index = Element
Element.__type = "Divider"

-- Idx is repurposed as the optional title string (no Config needed)
function Element:New(Title)

	local DividerRoot = New("Frame", {
	...
	if Title and Title ~= "" then
		local LeftLine = New("Frame", {
			...
			Size = UDim2.new(0.5, 0, 0, 1),   -- corrected once label sizes
			BackgroundTransparency = 0.5,
			Parent = DividerRoot,
		...
		-- Adjust segment lengths so each one ends/starts SIDE_GAP px from the label
		local function UpdateSegments()
			local labelHalf = Label.AbsoluteSize.X / 2
			local totalWidth = DividerRoot.AbsoluteSize.X
			if totalWidth == 0 then return end

			local segWidth = (totalWidth / 2) - labelHalf - SIDE_GAP
			if segWidth < 0 then segWidth = 0 end

			LeftLine.Size  = UDim2.new(0, segWidth, 0, 1)
			RightLine.Size = UDim2.new(0, segWidth, 0, 1)
		end

		Label:GetPropertyChangedSignal("AbsoluteSize"):Connect(UpdateSegments)
		DividerRoot:GetPropertyChangedSignal("AbsoluteSize"):Connect(UpdateSegments)
	else
		-- Plain line — no label
		New("Frame", {
			AnchorPoint = Vector2.new(0, 0.5),
...
function Element:New(Idx, Config)
	...
	local DropdownFrame = __require("Components.Element")(Config.Title, Config.Description, self.Container, false)
...
function Element:New(Idx, Config)
	...
	local InputFrame = __require("Components.Element")(Config.Title, Config.Description, self.Container, false)

	Input.SetTitle = InputFrame.SetTitle
	Input.SetDesc = InputFrame.SetDesc

	local Textbox = __require("Components.Textbox")(InputFrame.Frame, true)
...
function Element:New(Idx, Config)
	...
	local KeybindFrame = __require("Components.Element")(Config.Title, Config.Description, self.Container, true)
...
function Paragraph:New(Config)
	...
	local Paragraph = __require("Components.Element")(Config.Title, Config.Content, Paragraph.Container, false)
...
function Element:New(Idx, Config)
	...
	-- Track the touch ID so we only respond to the first finger
	local ActiveTouchId = nil

	local SliderFrame = __require("Components.Element")(Config.Title, Config.Description, self.Container, false)
	...
	-- Larger invisible hit area for the dot (easier to grab on touch)
	...
	-- Dot scale animation: grows when grabbed, shrinks when released
	local function SetDotActive(active)
		TweenService:Create(
			SliderDot,
			TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
			{ Size = active and UDim2.fromOffset(18, 18) or UDim2.fromOffset(14, 14) }
		):Play()
	end

	-- Compute value from an absolute X position
	local function ComputeValueFromX(absX)
		local SizeScale = math.clamp(
			(absX - SliderRail.AbsolutePosition.X) / SliderRail.AbsoluteSize.X,
			0, 1
		)
		return Slider.Min + ((Slider.Max - Slider.Min) * SizeScale)
	end
	...
	-- Touch drag: use InputBegan on the enlarged hitbox
	...
	Creator.AddSignal(SliderDotHitbox.InputEnded, function(Input)
	...
	-- Also allow tapping directly on the rail to jump the value (touch-friendly)
	Creator.AddSignal(SliderInner.InputBegan, function(Input)
		if Input.UserInputType == Enum.UserInputType.Touch or Input.UserInputType == Enum.UserInputType.MouseButton1 then
			local newVal = ComputeValueFromX(Input.Position.X)
	...
	Creator.AddSignal(SliderInner.InputEnded, function(Input)
		if Input.UserInputType == Enum.UserInputType.Touch or Input.UserInputType == Enum.UserInputType.MouseButton1 then
			if Input == ActiveTouchId or Input.UserInputType == Enum.UserInputType.MouseButton1 then
				ActiveTouchId = nil
				Dragging = false
				SetDotActive(false)
			end
	...
	Creator.AddSignal(UserInputService.InputChanged, function(Input)
		...
		then
			Slider:SetValue(ComputeValueFromX(Input.Position.X))
		end
	end)
...
-- Touch improvement: durations and feedback constants
local TWEEN_DURATION = 0.25
local TOUCH_RIPPLE_DURATION = 0.35
local LONG_PRESS_THRESHOLD = 0.4 -- seconds
...
function Element:New(Idx, Config)
	...
	local ToggleFrame = __require("Components.Element")(Config.Title, Config.Description, self.Container, true)
	...
	-- Touch ripple effect frame (appears on tap)
	...
	-- Plays a brief white ripple flash on touch/click
	local function PlayRipple()
		RippleFrame.BackgroundTransparency = 0.7
		TweenService:Create(
			RippleFrame,
			TweenInfo.new(TOUCH_RIPPLE_DURATION, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
			{ BackgroundTransparency = 1 }
		):Play()
	end
	...
	function Toggle:SetValue(Value)
		...
		TweenService:Create(
			ToggleCircle,
			TweenInfo.new(TWEEN_DURATION, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
	...
	-- Touch & mouse handling with ripple feedback
	local touchStartTime = 0
	local isTouching = false
	...
	Creator.AddSignal(ToggleFrame.Frame.InputEnded, function(Input)
		if
			(Input.UserInputType == Enum.UserInputType.Touch
			or Input.UserInputType == Enum.UserInputType.MouseButton1)
			and isTouching
		then
			isTouching = false
			local elapsed = tick() - touchStartTime
			-- Only toggle on short tap (not long press), prevents accidental triggers
			if elapsed < LONG_PRESS_THRESHOLD then
				PlayRipple()
	...
	-- Also keep MouseButton1Click for backwards compatibility (desktop)
	Creator.AddSignal(ToggleFrame.Frame.MouseButton1Click, function()
		-- InputBegan/Ended already handles this for mouse too, but keeping
		-- this as a fallback for reliability
	end)
...
for _, Theme in next, {__require("Themes.Amethyst"), __require("Themes.Aqua"), __require("Themes.Dark"), __require("Themes.Darker"), __require("Themes.Light"), __require("Themes.Rose")} do
...
local Acrylic = __require("Acrylic")
-- (bundled: Components resolved via __require)
local NotificationModule = __require("Components.Notification")
