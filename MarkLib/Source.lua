...
local function makeElement(parent, opts, extraHeight)
  local h = extraHeight or 34
  local row = new("Frame", {
    Parent = parent,
  ...
  local titleLabel = new("TextLabel", {
    ...
    Size = UDim2.new(1, -60, 0, opts.Desc and 20 or h),
    Text = opts.Title or "",
    TextColor3 = Color3.fromRGB(210, 210, 220),
    TextSize = 12,
    Font = Enum.Font.GothamBold,
    TextXAlignment = Enum.TextXAlignment.Left,
    TextYAlignment = opts.Desc and Enum.TextYAlignment.Bottom or Enum.TextYAlignment.Center
  })
...
function MarkLib:Window(cfg)
  ...
  local file      = cfg.SaveFile or "config.txt"
  ...
  function Win:MakeTab(opts)
    ...
    local tabFlag  = opts.Flag or tabTitle
    ...
    local function getParent(sectionFrame)
      return sectionFrame or panel
    end
    ...
    function Tab:Section(opts)
      ...
      new("UIStroke", { Parent = sFrame, Color = Color3.fromRGB(38, 38, 48), Thickness = 1 })
      ...
      function Section:Toggle(o)
        Tab:Toggle(o, sectionItems)
      end

      function Section:Button(o)
        Tab:Button(o, sectionItems)
      end
    ...
    function Tab:Toggle(opts, customParent)
      ...
      local h      = opts.Desc and opts.Desc ~= "" and 46 or 34
      local target = customParent or panel

      if flag then MarkLib.Flags[flag] = tState end

      local row = makeElement(target, opts, h)
      ...
      local function updateVisual()
        track.BackgroundColor3 = tState
          and Color3.fromRGB(75, 175, 95)
          or  Color3.fromRGB(45, 45, 58)
        thumb.Position = tState
      ...
      row.InputBegan:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.Touch then
          tState = not tState
          updateVisual()
          if opts.Callback then opts.Callback(tState) end
        end
      end)
    ...
    function Tab:Button(opts, customParent)
      ...
      local row = makeElement(target, opts, h)
      new("UIStroke", { Parent = row, Color = Color3.fromRGB(38, 38, 50), Thickness = 1 })
